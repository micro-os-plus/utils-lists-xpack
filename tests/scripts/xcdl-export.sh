...
# build-helper/templates/common/_micro-os-plus/tests/scripts/xcdl-export-liquid.sh
#
...
{
  xcdl_jsonc_path="$1"
  output_folder_path="$(dirname "${xcdl_jsonc_path}")"
  ...
  serialise_string_property_to "xcdl_context" "jsonFilePath" \
        "${xcdl_jsonc_path}" "xcdl_"

  serialise_string_property_to "xcdl_context" "outputFolderPath" \
        "${output_folder_path}" "xcdl_"
  ...
  substitute "${script_folder_path}/templates/CMakeLists-liquid.txt" "${output_folder_path}/CMakeLists.txt"
  substitute "${script_folder_path}/templates/meson-liquid.build" "${output_folder_path}/meson.build"
}
...
if [[ $# -lt 1 ]]
then
  echo "Usage: $(basename "$0") <xcdl-package.jsonc> [<xcdl-package.jsonc>...]"
  exit 1
fi
