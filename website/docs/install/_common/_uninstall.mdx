{/* DO NOT EDIT! Automatically generated from template file: */}
{/* docusaurus-template-liquid/templates/common/docs/install/_common/_uninstall-liquid.mdx */}

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import CodeBlock from '@theme/CodeBlock';

import LinksVsForwarders from '@site/src/components/LinksVsForwarders';

import CommonXpmCleanUps from '@site/docs/_common/_xpm-clean-ups.mdx';

{/* ------------------------------------------------------------------------ */}

### Uninstall

The xpm packages do not utilise any form of system installer; instead they
are distributed as
portable archives;
therefore they do not require the execution of any uninstaller; simply removing the
links and possibly the user's global xPacks store folder and
the user xPack cache folder is sufficient.

To remove the links created by
**xpm** in the current project,
navigate to the project folder:

```sh
cd my-project
```

and request **xpm** to uninstall the package:

<CodeBlock language="sh"> {
`xpm uninstall @micro-os-plus/utils-lists --verbose
`} </CodeBlock>

To completely remove the package from the user's global xPacks store:

<CodeBlock language="sh"> {
`xpm uninstall --global @micro-os-plus/utils-lists --verbose
`} </CodeBlock>

<details>
<summary>Clean-ups</summary>

<CommonXpmCleanUps />

</details>
